...
POT_PROVIDER_URL = os.environ.get("YTDLP_POT_PROVIDER_URL", "http://127.0.0.1:4416").strip()
...
def ytdlp_options(workdir, fmt):
    # Current yt-dlp guidance recommends mweb + a PO-token provider for
    # YouTube GVS requests. The provider creates a fresh token as needed.
    common = {
...
def convert():
    ...
    try:
        opts = ytdlp_options(workdir, fmt)
        ...
        # Cleanup only after Flask/WSGI has finished sending the file.
        ...
        if "sign in to confirm" in lower or "confirm you're not a bot" in lower:
            message = (
                "YouTube blokkeert de serveraanvraag met een bot/login-challenge. "
                "Een PO-token kan dit niet voor iedere video of server-IP oplossen."
            )
