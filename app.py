...
def build(src, dst, fmt, q, hv, ha):
    ...
    if fmt=="mp4":
        if hv:
            cmd=["ffmpeg","-y","-i",str(src),"-map","0:v:0","-map","0:a:0?","-c:v","libx264","-preset","veryfast","-crf",crf,"-c:a","aac","-b:a","160k","-movflags","+faststart"]
        else:
            cmd=["ffmpeg","-y","-f","lavfi","-i","color=c=black:s=1280x720:r=25","-i",str(src),"-shortest","-map","0:v:0","-map","1:a:0","-c:v","libx264","-preset","veryfast","-tune","stillimage","-pix_fmt","yuv420p","-c:a","aac","-b:a","160k","-movflags","+faststart"]
        return cmd+["-progress","pipe:1","-nostats",str(dst)]
    if fmt=="webm":
        if not hv: raise ValueError("WebM is een videoformaat; kies een audioformaat.")
        return ["ffmpeg","-y","-i",str(src),"-map","0:v:0","-map","0:a:0?","-c:v","libvpx-vp9","-crf",{"high":"30","standard":"34","small":"40"}[q],"-b:v","0","-c:a","libopus","-b:a","128k","-progress","pipe:1","-nostats",str(dst)]
    if fmt=="mkv":
        if not hv: return ["ffmpeg","-y","-i",str(src),"-vn","-c:a","copy","-progress","pipe:1","-nostats",str(dst)]
        return ["ffmpeg","-y","-i",str(src),"-c:v","libx264","-preset","veryfast","-crf","23","-c:a","aac","-b:a","160k","-progress","pipe:1","-nostats",str(dst)]
    if fmt=="mov":
        if not hv: raise ValueError("MOV is een videoformaat; kies een audioformaat.")
        return ["ffmpeg","-y","-i",str(src),"-map","0:v:0","-map","0:a:0?","-c:v","libx264","-preset","veryfast","-crf","23","-c:a","aac","-b:a","160k","-movflags","+faststart","-progress","pipe:1","-nostats",str(dst)]
    if fmt=="avi":
        if not hv: raise ValueError("AVI is een videoformaat; kies een audioformaat.")
        return ["ffmpeg","-y","-i",str(src),"-map","0:v:0","-map","0:a:0?","-c:v","mpeg4","-q:v","4","-c:a","aac","-b:a","160k","-progress","pipe:1","-nostats",str(dst)]
