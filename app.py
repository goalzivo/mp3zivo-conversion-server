...
def convert():
    ...
    # Current yt-dlp requires EJS + a supported JS runtime for YouTube.
    # The PyPI [default] extra installs yt-dlp-ejs; Deno is installed in Docker.
    common = {
        "outtmpl": template,
        "noplaylist": True,
        "quiet": True,
    ...
    try:
        ...
        if "sign in to confirm" in lower:
            message = (
                "YouTube blokkeert deze serveraanvraag met een login/challenge. "
                "De video is niet zonder geautoriseerde sessie beschikbaar."
            )
            ...
            message = "Deze video is privé en vereist geautoriseerde toegang."
        elif "confirm you're not a bot" in lower or "not a bot" in lower:
            message = (
                "YouTube blokkeert geautomatiseerde toegang vanaf deze server. "
                "Probeer later opnieuw."
            )
        else:
            message = f"YouTube kon de video niet verwerken: {raw[:400]}"
